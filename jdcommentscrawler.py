...
class JDCommentsCrawler:
    ...
    def __init__(self,productId=None,callback=None,page=0,score=1,sortType=5,pageSize=10):
    ...
    def showList(self):
        ...
        return_str = conn.read().decode('gbk','ignore')
    ...
    def crawler(self):
        ...
        for p in range(self.page-1,100):  
            ...
            for com in comments:
                tmp_list.append([com['id'],productId,com['guid'],com['content'],com['creationTime'],com['referenceId'],com['referenceTime'],com['score'],\
                                 com['nickname'],com['userLevelName'],com['content'],com['isMobile'],com['userClientShow']])
            df = pd.DataFrame(tmp_list)
            with open('d:\\k.csv','a') as csvfile:
                df.to_csv(csvfile)
            time.sleep(1)
...
def jdComment():
	#设置关键变量
	page=1
	productId =5523403 # 商品ID
	callback = 'fetchJSON_comment98vv1300' #回调函数
